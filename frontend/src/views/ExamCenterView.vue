...
function handleCreated(exam: ExamRecord) {
  wizardVisible.value = false
  router.push(`/exam-center/${exam.id}/scores`)
}
...
<template>
  <section class="gm-management-page">
    <div class="gm-page-header">
      <div>
        <h1>考试中心</h1>
        <p>创建考试、进入成绩录入，并查看每场考试的处理状态。</p>
      </div>
      <el-button type="primary" @click="wizardVisible = true">创建考试</el-button>
    </div>
    ...
    <el-dialog v-model="wizardVisible" title="创建考试" width="860px">
      <ExamWizard @created="handleCreated" />
    </el-dialog>
  </section>
</template>
