...
describe('table layout conventions', () => {
  ...
  it('contains the global CSS needed for contained scrolling and lightweight scan cues', () => {
    ...
    expect(css).toMatch(/\.gm-score-metrics\s*{[^}]*grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*160px\)\)/s)
    expect(css).toMatch(/\.gm-score-metrics\s*{[^}]*justify-content:\s*start/s)
    ...
    expect(css).toMatch(/\.gm-donut\s*{[^}]*width:\s*112px/s)
